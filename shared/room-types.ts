...
export type Room = { code: string; hostId: string; phase: RoomPhase; players: Player[]; settings: Settings; customQuestions: Record<string, string[]>; questions: string[]; roundResults: RankingResult[][]; cumulativeScores: Record<string, number>; updatedAt: number; countdownEndsAt?: number; revealIndex?: number; questionIndex?: number };
...
export type RoomState = Omit<Room, 'customQuestions'> & { questionProgress: number; questionTarget: number; totalQuestions: number; results?: RankingResult[] };
