...
export type RoomPhase = 'lobby' | 'question_draft' | 'ranking' | 'waiting' | 'reveal' | 'finished';
...
export type Room = { code: string; hostId: string; phase: RoomPhase; players: Player[]; settings: Settings; customQuestions: Record<string, string[]>; updatedAt: number };
export type RankingSubmission = { playerId: string; order: string[] };
export type RoomState = Omit<Room, 'customQuestions'> & { questionProgress: number; questionTarget: number };
