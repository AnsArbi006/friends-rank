...
export type RankingResult = { playerId: string; points: number; totalPoints: number; percent: number; firstPlaceVotes: number; correctAnswers?: number; fastestCorrectMs?: number | null };
export type RoomState = Omit<Room, 'customQuestions' | 'guessQuestions' | 'guessAuthorOrder' | 'guessAuthorId' | 'guessAnswers' | 'guessRoundScores' | 'guessCumulativeScores' | 'guessStartedAt'> & { questionProgress: number; questionTarget: number; totalQuestions: number; results?: RankingResult[]; guessRole?: 'author' | 'participant'; guessQuestionText?: string; guessOptions?: string[]; guessDeadline?: number; guessReveal?: { authorId: string; targetId: string; text: string; scores: Record<string, number>; stats: Record<string, { correct: number; fastestCorrect: number | null }> } };
